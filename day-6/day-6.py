...
# other Elves that it would be okay to give you their one malfunctioning device - surely you'll have no problem
# fixing it.
#
...
# first position where the four most recently received characters were all different. Specifically, it needs to
# report the number of characters from the beginning of the buffer to the end of the first such four-character marker.
def parse_stream():
    input_file = open('input.txt', 'r')
    return list(input_file.readline().removesuffix('\n'))
...
def find_start_of_message():
    ...
    for i in range(len(stream)):
        position += 1
        message_packet = stream[i:i+14]
        if len(set(message_packet)) == 14:
            break

    return position + 14


print(find_start_of_packet())
print(find_start_of_message())
